...
// Performs the simulation per run. See run() method to see the steps in each run.
...
private:
    ...
    // The state of the city. (A memopad that records where each resident lives in the city.)
    std::unique_ptr<CityState> _city_state;

    // All residents and they are sorted.
    ...
    // Normal run is all runs that aren't the first run. See run() for how a normal run
    // progresses.
    void normalRun ();

#include "YUpAxisUtility.h"
#include "TextRect.h"

YUpAxisUtility::YUpAxisUtility (
    std::string title,
    Renderer* renderer,
    ...
    _title_font_size{titleFontSize}
{
    ...
    _bottom_most_pixel = _y_coord__px;
    _top_most_pixel = _pc->getPixel(_max_val) + _end_offset__px;

    _tick_width__px = (_cell_size__px % 2 == 0) ? 2 : 1;

}

void YUpAxisUtility::bottom2TopTitleOnLeft ()
{
    ...
    while (currValue <= _pc->getValue(_top_most_pixel))
    {
